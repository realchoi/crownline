...
截至 2026-08-18，当前数据已为数据集内全部 71 个中国政权和 43 个世界政权接入统治者详情，共收录 1202 位人物、1241 条任期、18 条结构化关系、14 条事件、100 条地理快照和 131 项来源；中国全览仍保持 73 条时间轴记录、7 个历史阶段。收录范围以正式统治者为主，并包括有来源支持的重要摄政者、共治者和主要争位者。早期王年只录入可明确披露史学口径的记录，其余年份保留为“资料尚未校订”；只有存在有来源支持的 `reignVacancies` 时，才可显示明确“空位”，否则应视为资料缺口而不是历史断言。世界样本现覆盖东亚、东南亚、中亚、南亚、西亚、欧洲、北非、西非、东非、南部非洲与美洲，但仍只是当前数据集的代表性样本，不代表完整通史覆盖。关系仍只代表已校订案例，不表示任意政权组合都已完成关系整理；每条关系与事件的来源引用必须提供章节、页码或页面内定位，避免以通用政权概览替代事件证据。
