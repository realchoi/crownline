...
截至 2026-08-22 的扩容批次和地图样本说明保留在历史记录中；当前可机器校验的数据摘要见上方标记区块。
...
历史进展记录（2026-08-22）：本批次完成全球样本的统治者、关系、事件、地理快照与来源扩容；大津巴布韦王国与特奥蒂瓦坎目前没有可展示的统治序列，保持“资料尚未校订”口径。收录范围以正式统治者为主，并包括有来源支持的重要摄政者、共治者和主要争位者。现有扩容口径继续保留各地区的阶段边界、共治与史料不确定性；早期王年只录入可明确披露史学口径的记录，其余年份保留为“资料尚未校订”；只有存在有来源支持的 `reignVacancies` 时，才可显示明确“空位”，否则应视为资料缺口而不是历史断言。关系仍只代表已校订案例，不表示任意政权组合都已完成关系整理；每条关系与事件的来源引用必须提供章节、页码或页面内定位，避免以通用政权概览替代事件证据。当前总数以本节上方机器维护区块为准。
