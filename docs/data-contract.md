...
历史进展记录（2026-08-25）：新增 20 条按历史网络策展的关系，而非从全部政权组合中随机抽取。四组网络分别为中国分裂时期、丝绸之路与中亚、地中海—西亚、海上亚洲与印度洋。每条关系仍必须指向两个确实在所记录时期存续的政权，并由来源直接支持双方、类型和时间口径；考古商品和远程贸易证据只建立网络联系，不升级为中央政府间条约。关系报告当前记录 48 条案例、51 个参与政权，参与比例仍不是完成率。
