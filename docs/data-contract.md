...
2026-09-08 的玛雅建模审查将原“玛雅城邦（古典期）”改为 `historical-period` 背景分期，并新增蒂卡尔、帕伦克、卡拉克穆尔和科潘四个 `polity`。背景分期没有人物、任期、关系或地图点位；这些记录只归入来源直接支持的具体政治体。四个城邦各自的采用起止年仍保留约年或争议说明，不能从古典期约250—900年的分期边界推出任一城邦的统一存续期。
...
- 关系和事件的每个区间都必须完整落在每个参与实体的某一段存续期内，不能早于建国、晚于终结或跨越存续中断期；构建期拒绝越界记录。参与者在建国前或灭亡后的相关活动只在摘要中说明，不为凑区间改写实体存续期。
...
历史进展记录（2026-08-25）：新增 20 条按历史网络策展的关系，而非从全部政权组合中随机抽取。四组网络分别为中国分裂时期、丝绸之路与中亚、地中海—西亚、海上亚洲与印度洋。每条关系仍必须指向两个确实在所记录时期存续的政权，并由来源直接支持双方、类型和时间口径；考古商品和远程贸易证据只建立网络联系，不升级为中央政府间条约。

历史进展记录（2026-09-03）：第二轮跨地区网络新增 26 条关系与 16 个事件，其中非战争关系 14 条；东亚外交与文化、印度洋贸易、欧亚宫廷、非洲和美洲网络均新增参与者。关系报告当前记录 74 条案例、76 个参与政权；参与比例 58.02% 仍只描述已收录案例分布，不是关系完成率。争议军事介入使用 `medium` 或 `disputed` 并提供 `confidenceNote`；长期贸易区间继续明确中介网络和非中央政府持续经营边界。

历史进展记录（2026-09-08）：古典期玛雅诸城邦聚合条目改为历史分期后，378年特奥蒂瓦坎相关“进入”事件与两条关系均收窄到蒂卡尔，避免从单一遗址证据外推全部玛雅政治体。蒂卡尔、帕伦克、卡拉克穆尔和科潘只录入铭文或机构资料直接支持的代表君主；其余年份继续是“统治者资料未知”，不是明确空位。朝鲜王朝、琉球王国与阿瑜陀耶王国则分别按机构王表补齐26、25、33位统治者，消除原抽样序列中的116、249、159个未知政权年，共524年；同年交接仍按闭区间保留年内先后在位者。

历史进展记录（2026-09-23）：第三轮关系网络新增51条关系，总数达到125条、参与政权105个；参与比例78.36%仍只描述已收录案例分布，不是关系完成率。中国相关关系优先引用正史与《资治通鉴》原文，locator 写到卷、纪年与关键记事，区间只覆盖原文直接记载的年份，例如后金—明只记天命三年至四年、唐—南诏战争只记天宝十年。南诏—吐蕃与后晋—辽两条臣属关系在摘要中说明被臣属方保留本国王统或帝号；宋—高丽、宋—大理、金—高丽只记朝贡或称藩框架，不写成政治臣属。来源只给出君主在位年代的萨珊—隋遣使使用约年并标为 `medium`；图尔之战按来源“名义宗主权”表述标为 `medium`，不表示大马士革中央直接组织此役。

历史进展记录（2026-09-23，遗留口径修订）：关系与事件校验从“与参与方存续期相交”收紧为“每个区间完整落在每个参与方的某一段存续期内”。全量扫描只发现蒙古—西夏战争一条越界：据《元史》卷一，1205年（乙丑）征西夏早于1206年大蒙古国建立，关系区间改为1207—1227，1205年攻掠只在摘要中说明。西辽保留 Biran《牛津亚洲史研究百科》的1124—1218采用口径，另以 `alternativeChronologies` 披露 Encyclopaedia Iranica 以1211年屈出律篡位为终结的口径，并在年代说明中列出《辽史》卷三十“直鲁古死，辽绝”与屈出律卒年1217/1218的差异；西辽—花剌子模臣属止于1210年、卡特万之战和巴拉沙衮点位在两种口径下均无需调整。

同日补录上一轮暂缺来源的7条关系，总数达到132条、参与政权108个。德川—琉球只记江户遣使的外交礼仪，摘要披露琉球1609年后实际受萨摩藩支配并维持对中国的朝贡册封，不写成对幕府的臣属；英格兰—神圣罗马同盟只代表遭绝罚、帝位受腓特烈二世挑战的奥托四世一方，标为 `medium`；埃塞俄比亚—奥斯曼关系的奥斯曼一方是哈贝什行省，通好起点按总督任命日期推定为约1583年；占婆—高棉只记《宋史》直接记载的1177年，驱逐年份不入区间。
...
`temporalCoverage` 在工具侧按实体 ID 稳定列出存续期、正式统治者/共治、摄政、争位、全部任期、明确空位和点位的年份区间及计数。区间使用 `{startYear, endYear}` 闭区间，按政权存在期裁切后求并集，跳过公元0年，共治和迁都重叠只计一次。正式任期与明确空位之外为正式统治者资料未知区间；仅有摄政或争位记录不能消除此未知状态。未知不表示无人统治。

约年与争议沿用源数据采用端点，覆盖百分比不将其升级为精确事实。跨政权汇总按“政权年”累计，不是全球不重复年份。原先 `available` 仍仅表示至少有可展示记录；两种指标不能混称。细节、首批五个候选的处理和修改前后区间见 [年份覆盖审查](archive/temporal-coverage-review.md)。

2026-09-08 补订后，朝鲜王朝、琉球王国与阿瑜陀耶王国的 `rulerDetails.unknownYears` 均为0；这只表示采用王表在年度粒度下连续，并不保证日月级交接或所有摄政、共治与争位角色都已穷尽。玛雅拆分新增四个真实政权及各自政权年分母，因此全局正式任期覆盖比例会同时受到模型粒度变化影响，不能用拆分前后的单一百分比判断本批补订是否有效。
...
地理证据进展（2026-09-08，P0）：对审查基线中100条所有引用均缺少 `locator` 的既有点位建立独立维护档案。本轮闭环蒂卡尔及8条中国点位，另有91条标为 `pending-evidence`，逐条记录坐标、历史地点身份和适用年代所缺证据。西汉长安起点纠正为前200年，隋大兴起点纠正为583年，唐长安终点纠正为904年，元大都起点纠正为1272年；金中都区分1214年迁汴与1215年城破，点位止于迁都年。当前191条生产点位中100条至少有一项非空 `locator`，其中30条全部引用已定位、70条仍有部分引用待补。`pending-evidence` 不表示记录已判错，也不能用通用 GeoNames 首页或政权纪年自动补成已核实。

分期地理进展（2026-09-08，P1）：前燕补棘城337—342与邺城357—370后达到34/34政权年；西秦按勇士城、金城、苑川、度坚山、谭郊、枹罕和南安拆分后覆盖两段39/39政权年，401—408中断期没有点位。南明补桂林1647、南宁1649—1651、安龙1652—1656、昆明1656—1659，达到15/19，保留1648及1660—1662流动、流亡与拘押阶段缺口。神圣罗马帝国只新增1663—1806年永久帝国议会驻雷根斯堡的 `political-center`，不把它写成固定皇都；1025—1662年的巡回宫廷与多中心结构继续保留缺口。具体取舍与来源见[第二批分期地图点位审查](archive/temporal-geography-p1-review.md)。

地理证据与年份进展（2026-09-10，P0/P1）：第二批闭环大城、素可泰、吴哥、蒲甘、法泰赫普尔西格里、撒马尔罕、开京、江华、君士坦丁堡、布尔萨、开罗、阿克苏姆、大津巴布韦、特诺奇蒂特兰、库斯科和特奥蒂瓦坎16条既有点位；每条分别定位机构/学术史料与具体 OSM 对象。新增隋581—582年长安旧城、唐904—907年洛阳和元1271—1364年上都，隋、唐、元地理年份覆盖达到100%；南宋、南明与花剌子模的流动阶段继续保留未知。当前194条点位中119条至少有一项定位，49条全部引用已定位、70条部分定位、75条完全缺少定位。详见[第二批地理点位证据审查](archive/geography-evidence-p2.md)。

全记录来源定位进展（2026-09-15，P0）：12个核心实体、拜占庭与高丽的130位人物和133条任期，以及8条跨地区点位完成定位。无定位实体、人物、任期和点位分别降至102、1067、1103和67条；地理记录现有57条全部引用定位、70条部分定位。证据不足的点位年份保持未知；详见[第一批全记录来源定位](archive/source-evidence-p0.md)。

第二批来源定位进展（2026-09-15，P0）：再闭环巴格达、尼西亚、德里、贡德尔、埃迪尔内、大不里士、加兹温与哈兰，并补齐刘宋、南齐、梁、陈四条建康点位的坐标引用。无定位点位降至59条，部分定位降至66条，全部引用定位增至69条；门禁开始同时约束无定位与部分定位数量。巴格达排除836—892年萨迈拉首都期，贡德尔起点改为1636年，尼西亚延至1261年，德里改为代表性中心；详见[第二批来源定位](archive/source-evidence-p0-2.md)。

第三批来源定位进展（2026-09-16，P0）：补齐洛阳3条、开封4条、成都3条与姑臧2条点位的具体OSM坐标对象，不改变历史身份、角色或适用期。部分定位点位由66降至54，全部引用定位由69增至81；详见[第三批来源定位](archive/source-evidence-p0-3.md)。

第四批来源定位与误配纠正（2026-09-23，P0）：东吴武昌改用今鄂州现代城市级坐标，补入孙权迁建业年和孙皓复都段，并披露266/267年终年口径差异；无定位点位由59降至58。拜占庭与高丽共263条人物及任期引用改用具名王表条目，防止同页通用定位文本掩盖不可复核的引用；详见[第四批来源定位](archive/source-evidence-p0-4.md)。

萨迈拉分期点位（2026-09-23，P1）：按 UNESCO 遗产说明补入阿拔斯哈里发836—892年萨迈拉第二都城点位，并采用遗产地中心区登记坐标作近似区域锚点。巴格达点位继续排除该段，750—761年仍为未知；阿拔斯地理年份覆盖由440/509增至497/509，详见[萨迈拉分期点位审查](archive/temporal-geography-p2-review.md)。
...
2026-09-07 的 P0 审查将原 8 条试点快照全部退出生产；当前 `boundaries.json` 保留合法的空快照和空来源数组。原始记录保存在工具侧审查档案中，不进入 `CrownlineData` 或任何浏览器包。生产疆域必须通过独立证据门禁：有唯一的批准审查、可定位的上游对象及版本、许可核验和可复核的派生记录；通用版权页和通史来源不能代替几何证据。具体审查字段、逐条结论与恢复要求见 [疆域证据审查](boundary-pilot.md)。
...
独立的 `reviews/geography-locator-evidence-review.json` 保存地理引用证据审查，不进入 `CrownlineData` 或浏览器包。审查把坐标证据、历史地点/角色证据与适用年代证据分别记录；只有业务点位实际补入非空、可核查的 `locator` 后，覆盖报告才计入 `recordsWithLocatedSourceRefs`。截至2026-09-23，195条点位中137条至少有一项定位，58条仍完全缺少定位；至少一项定位并不等于全部引用已闭环，当前83条达到全部引用定位、54条仍为部分定位。未核实记录继续保留具体缺口和后续要求，不从同政权的其他来源自动推定。
