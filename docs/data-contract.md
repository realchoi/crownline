...
`npm run generate:data` 按 `order` 聚合实体分片，执行 JSON Schema 和跨记录语义校验，然后生成首屏 `index.json` 与按实体加载的详情包。`.generated/` 和 `public/data/generated/` 都是可重建产物，不得手工修改或提交。浏览器的窄校验只防止部署缺失或文件错配，不能替代生成阶段的全量校验。

截至 2026-08-12，当前数据集共包含 93 个实体，并可生成 93 个详情包；其中 71 个中国政权、20 个世界代表政权与 2 个中国历史分期共同组成中国全览的 73 条时间轴记录和 7 个历史阶段基线。
...
构建期必须完整执行上述 Schema 与语义校验，错误数据不得发布。运行时仍严格校验详情根结构、实体 ID、人物、任期、空位和来源闭包；关系与事件数组作为不可信边界交给领域层逐条收窄，单条异常会被跳过并提示，不会放宽构建期标准或破坏同页其他有效记录。
