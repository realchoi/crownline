...
Crownline（王冠纪）是一个面向全球历史的交互式王朝图谱项目。当前版本以中国历代王朝与主要政权为完整基线，并用少量外部代表条目验证跨地区时间点浏览；世界其他地区仍会在后续版本中逐步补全。

## 当前功能

- 完整全览按七个历史阶段展示 73 个中国历代王朝、主要政权与历史分期
- 支持全览与时间点两种模式，可站在指定历史年份查看当时存在的政权
- 时间点模式可切换中国、自选多个地区与全球已收录，并把地区状态同步到 URL
...
长远规划见 [ROADMAP.md](ROADMAP.md)。阶段 2 已完成中国、自选地区与全球已收录三种时间点范围，并让地区与年份、搜索和类别筛选组合生效。下一步阶段 2B 将把自选地区和全球已收录扩展到完整全览时间轴。
