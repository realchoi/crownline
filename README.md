...
- 支持时间轴与离线历史示意地图切换；地图默认汇集所有已校订政权点位，拖动年份轴后按当前年份筛选，并与地区、搜索、详情和双政权对比共享状态。现有地理数据覆盖 43 个世界样本与 26 个中国代表政权的 100 条有来源点位；点位只表示都城、政治中心或浏览定位，不代表完整疆域。
...
- 43 个世界政权样本已具备详细简介与来源，其中 41 个已接入统治者任期；大津巴布韦王国与特奥蒂瓦坎暂保留“资料尚未校订”口径，不把缺少可比的统治序列误写成无人统治
...
- 71 个中国政权与 41 个世界政权已接入统治者任期；当前数据集累计 116 个实体、1202 位人物、1241 条任期、18 条关系、14 条事件、100 条地理快照与 131 项来源，数据契约为 v4
...
当前数据集共包含 116 个实体、1202 位人物、1241 条任期、18 条结构化关系、14 条事件、100 条地理快照、131 项来源和 116 个详情包；其中中国全览仍保持 73 条时间轴记录、7 个历史阶段，71 个中国政权与 41 个世界政权已接入统治者任期。地图覆盖 43 个世界样本与 26 个中国代表政权，十一个顶层历史地区均有可显示点位；点位不代表疆域。美洲当前收录阿兹特克帝国、印加帝国、玛雅城邦（古典期）、托尔特克与特奥蒂瓦坎；西非收录马里帝国、桑海帝国与卡涅姆-博尔努帝国；东非收录阿克苏姆王国与埃塞俄比亚帝国；南部非洲收录大津巴布韦王国与穆塔帕帝国。除正式统治者外，还按来源收录重要摄政者、共治者和主要争位者。大津巴布韦王国与特奥蒂瓦坎目前没有可展示的统治序列，继续明确显示为“资料尚未校订”；其他早期王年也只录入可明确标注口径的代表记录。只有 `reignVacancies` 中存在有来源记录时才显示“空位”，避免从数据缺口推导历史事实。结构化关系覆盖七种类型，所有关系和事件来源均带章节、页码或页面定位；朝贡、臣属、贸易和文化交流均附限制性口径，不从数据缺口或时间重叠推断关系。
...
长远规划见 [ROADMAP.md](ROADMAP.md)。阶段 5 已完成点位式离线历史地图 MVP，时间轴与地图共享年份、地区、搜索、详情和双政权对比状态；带时间的疆域多边形仍是后续工作。阶段 4A 已支持双政权时间对比、多段区间求交、共同期统治者与 URL 恢复；阶段 4B 已接入按类型分组、可追溯且逐条容错的结构化历史关系。阶段 3 已扩展到数据集内全部 71 个中国政权和 41 个世界政权，共接入 1202 位人物与 1241 条任期；另两个世界样本保留明确的数据缺口提示。阶段 2B 的中国、自选地区与全球已收录三种范围继续共享全览和时间点状态，多地区全览按地区及跨地区政权分组，并在当前可见结果间使用统一时间比例。
