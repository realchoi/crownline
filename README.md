...
- 拜占庭、阿拔斯、神圣罗马和朱罗四个外部代表条目已具备详细简介、统治者任期与来源，并继续验证跨地区归属
- 明确区分尚未收录、覆盖有限和被搜索或类别筛选为空
- 支持传统纪年输入、逐年增减和连续滑杆，跨公元前后时自动跳过公元 0 年
- 时间点结果将真实政权与历史分期背景分开，并提示约年、争议与端点口径
- 71 个中国政权与 4 个世界代表政权已接入统治者任期；时间点详情可显示在位、共治、摄政、争位、明确空位与资料缺口
...
地区覆盖状态描述的是当前数据集，不是历史事实的完整程度。“全球已收录”只展示当前已有条目；外部地区目前仍只有少量具备详细信息的代表性条目。

统治者数据当前覆盖数据集内全部 71 个中国政权和拜占庭、阿拔斯、神圣罗马、朱罗 4 个世界代表政权，共收录 675 位人物与 706 条任期；其中世界条目包含 205 位人物和 236 条任期。除正式统治者外，还按来源收录重要摄政者、共治者和主要争位者。早期王年只录入可明确标注口径的代表记录，缺少任期的年份显示为“资料尚未校订”。只有 `reignVacancies` 中存在有来源记录时才显示“空位”，避免从数据缺口推导历史事实。
...
长远规划见 [ROADMAP.md](ROADMAP.md)。阶段 3 已从 16 个主线政权扩展到数据集内全部 71 个中国政权和 4 个世界代表政权，共接入 675 位人物与 706 条任期；阶段 2B 的中国、自选地区与全球已收录三种范围继续共享全览和时间点状态，多地区全览按地区及跨地区政权分组，并在当前可见结果间使用统一时间比例。
