...
- 支持时间轴与离线历史示意地图切换；地图按当前年份展示 43 个世界样本与 15 个中国代表政权的 87 条有来源点位，并与地区、搜索、详情和双政权对比共享状态。点位只表示都城、政治中心或浏览定位，不代表完整疆域。
...
- 71 个中国政权与 43 个世界政权已接入统治者任期；当前数据集累计 116 个实体、1202 位人物、1241 条任期、7 条关系、4 条事件、87 条地理快照与 126 项来源，数据契约为 v4
...
当前数据集共包含 116 个实体、1202 位人物、1241 条任期、7 条结构化关系、4 条事件、87 条地理快照、126 项来源和 116 个详情包；其中中国全览仍保持 73 条时间轴记录、7 个历史阶段，71 个中国政权与 43 个世界政权已接入统治者任期。地图覆盖 43 个世界样本与 15 个中国代表政权，十一个顶层历史地区均有可显示点位；点位不代表疆域。美洲当前收录阿兹特克帝国、印加帝国、玛雅城邦（古典期）、托尔特克与特奥蒂瓦坎；西非收录马里帝国、桑海帝国与卡涅姆-博尔努帝国；东非收录阿克苏姆王国与埃塞俄比亚帝国；南部非洲收录大津巴布韦王国与穆塔帕帝国。除正式统治者外，还按来源收录重要摄政者、共治者和主要争位者。早期王年只录入可明确标注口径的代表记录，缺少任期的年份显示为“资料尚未校订”。只有 `reignVacancies` 中存在有来源记录时才显示“空位”，避免从数据缺口推导历史事实。首批历史关系覆盖七种类型；朝贡、臣属、贸易和文化交流均附限制性口径，不从数据缺口或时间重叠推断关系。
...
只需系统装有 Python 3：首次运行会自动在项目内创建 `.venv-fonts/` 虚拟环境并安装 fonttools 与 brotli，无需手动 pip。检查失败时运行 `npm run regen:fonts`，以全量字体重新生成四个子集（脚本会自动下载字体源文件）；`names.local` 等外文原名暂未渲染，不在中文字体子集的覆盖范围内。
