...
Crownline（王冠纪）是一个面向全球历史的交互式王朝图谱项目。当前版本以中国历代王朝与主要政权为完整基线，并用覆盖东亚、东南亚、中亚、南亚、西亚、欧洲、北非、西非与美洲的 20 个世界代表政权样本验证跨地区时间点与完整全览浏览；世界其他地区仍会在后续版本中逐步补全。
...
- 20 个世界代表政权样本已具备详细简介、统治者任期与来源，覆盖东亚、东南亚、中亚、南亚、西亚、欧洲、北非、西非与美洲，并继续验证跨地区归属
- 明确区分尚未收录、覆盖有限和被搜索或类别筛选为空
- 支持传统纪年输入、逐年增减和连续滑杆，跨公元前后时自动跳过公元 0 年
- 时间点结果将真实政权与历史分期背景分开，并提示约年、争议与端点口径
- 71 个中国政权与 20 个世界代表政权已接入统治者任期；当前数据集累计 93 个实体、985 位人物、1024 条任期、7 条关系、4 条事件、59 条地理快照与 80 项来源，数据契约为 v4
...
地区覆盖状态描述的是当前数据集，不是历史事实的完整程度。“全球已收录”只展示当前已有的 20 个世界代表政权；这些条目已覆盖东亚、东南亚、中亚、南亚、西亚、欧洲、北非、西非与美洲，但仍只是代表性样本，不表示完整世界史已经补齐。

当前数据集共包含 93 个实体、985 位人物、1024 条任期、7 条结构化关系、4 条事件、59 条地理快照、80 项来源和 93 个详情包；其中中国全览仍保持 73 条时间轴记录、7 个历史阶段，71 个中国政权与 20 个世界代表政权已接入统治者任期。首批地图覆盖 20 个世界样本与 15 个中国代表政权，九个顶层历史地区均有可显示点位；点位不代表疆域。美洲已从“尚未收录”升级为 `sample`，当前收录阿兹特克帝国与印加帝国。除正式统治者外，还按来源收录重要摄政者、共治者和主要争位者。早期王年只录入可明确标注口径的代表记录，缺少任期的年份显示为“资料尚未校订”。只有 `reignVacancies` 中存在有来源记录时才显示“空位”，避免从数据缺口推导历史事实。首批历史关系覆盖七种类型；朝贡、臣属、贸易和文化交流均附限制性口径，不从数据缺口或时间重叠推断关系。
...
长远规划见 [ROADMAP.md](ROADMAP.md)。阶段 5 已完成点位式离线历史地图 MVP，时间轴与地图共享年份、地区、搜索、详情和双政权对比状态；带时间的疆域多边形仍是后续工作。阶段 4A 已支持双政权时间对比、多段区间求交、共同期统治者与 URL 恢复；阶段 4B 已接入按类型分组、可追溯且逐条容错的结构化历史关系。阶段 3 已扩展到数据集内全部 71 个中国政权和 20 个世界代表政权，共接入 985 位人物与 1024 条任期。阶段 2B 的中国、自选地区与全球已收录三种范围继续共享全览和时间点状态，多地区全览按地区及跨地区政权分组，并在当前可见结果间使用统一时间比例。
