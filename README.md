...
统治者数据当前覆盖数据集内全部 71 个中国政权和拜占庭、阿拔斯、神圣罗马、朱罗 4 个世界代表政权，共收录 672 位人物与 702 条任期；其中世界条目包含 202 位人物和 232 条任期。除正式统治者外，还按来源收录重要摄政者、共治者和主要争位者。早期王年只录入可明确标注口径的代表记录，缺少任期的年份显示为“资料尚未校订”。只有 `reignVacancies` 中存在有来源记录时才显示“空位”，避免从数据缺口推导历史事实。
...
长远规划见 [ROADMAP.md](ROADMAP.md)。阶段 3 已从 16 个主线政权扩展到数据集内全部 71 个中国政权和 4 个世界代表政权，共接入 672 位人物与 702 条任期；阶段 2B 的中国、自选地区与全球已收录三种范围继续共享全览和时间点状态，多地区全览按地区及跨地区政权分组，并在当前可见结果间使用统一时间比例。
