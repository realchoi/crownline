...
export async function generateData(
  ...
  return withGenerateDataLock(lockRoot, async () => {
    ...
    try {
      ...
      await writeJson(join(publicStaging, "boundaries.json"), artifacts.boundaries);
      await Promise.all(
    ...
    .then((summary) => {
      console.log(
        `数据生成完成：${summary.sections} 个阶段，${summary.entities} 个实体，` +
          ...
          `${summary.geographicSnapshots} 条地理快照，${summary.boundarySnapshots} 条疆域快照，` +
          `${summary.details} 个详情包，` +
          `${summary.sources} 个来源`
      );
    })
    .catch((error) => {
