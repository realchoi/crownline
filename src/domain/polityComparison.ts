...
export function intersectHistoricalPeriods(
  ...
  return leftPeriods
    ...
    .sort((left, right) => {
      return (
        toOrdinal(left.start.year) - toOrdinal(right.start.year) ||
...
export function buildPolityComparison(
  ...
  const overlapPeriods = intersectHistoricalPeriods(left.existencePeriods, right.existencePeriods);
  return {
    left,
    right,
    overlapPeriods,
...
export function selectRulersDuringPeriods(
  ...
  return detail.reigns
    ...
    .sort((left, right) => {
      return (
        toOrdinal(left.periods[0]!.start.year) - toOrdinal(right.periods[0]!.start.year) ||
