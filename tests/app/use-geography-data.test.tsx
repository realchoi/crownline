...
import type { ViewMode } from "../../src/domain/browseState";
...
describe("useGeographyData", () => {
  it("进入地图时懒加载并在切换视图后复用成功结果", async () => {
    ...
    const { result, rerender } = renderHook(
      ({ viewMode }: { viewMode: ViewMode }) => useGeographyData(viewMode, loadGeography),
  ...
  it("失败后可重试", async () => {
    ...
    const { result } = renderHook(() => useGeographyData("map", loadGeography));
