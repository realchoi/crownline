...
describe("多地区全览分组", () => {
  ...
  it("选择欧洲与西亚时只把同时命中的拜占庭移入跨地区组", () => {
    ...
    expect(groups.find(({ regionId }) => regionId === "region-europe")?.matches
      .map(({ entity }) => entity.names.primary)).toEqual(["神圣罗马帝国"]);
    expect(groups.find(({ regionId }) => regionId === "region-west-asia")?.matches
      .map(({ entity }) => entity.names.primary)).toEqual(["阿拔斯哈里发"]);
  });
  ...
  it("按起点、终点和名称稳定排序并使用组内局部范围", () => {
    ...
    expect(europe!.range).toEqual({ startYear: 330, endYear: 1806 });
    expect(europe!.displayRange).toBe("330—1806");
